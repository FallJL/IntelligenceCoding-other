...
#
#    Copyright (C) 2021  Jernej Pangerc; https://github.com/jernejp21
#
...
def generateGraph(functionName, flow):
    ...
    for element in flow:
        ...
        if elementID == 'fc:ifBranch' or  elementID == 'fc:ifMacro' or elementID == 'fc:forLoop':
    ...
    for node in nodes:
        ...
        if connectWith == None:
            ...
            if connectingNode['comment'] == 'fc:ifBranch' or connectingNode['comment'] == 'fc:ifMacro':
    ...
    if VIEW:
        ...
        filename = "python_flowChart_out/out/FC_"+uuid.uuid4().hex
        g.render(directory=DEST, format="png", filename=filename)
        return filename


def main(source,dest,lang):
    SOURCE = source
    DEST = dest
    LANG = lang
                        ...
                        if FUNCS:
                            for func in FUNCS:
                                if func == functionName:
                                    filename = generateGraph(functionName, flow)
                                    if filename:
                                        filepath.append(filename)
                                        filenames.append(functionName)
                        else:
                            filename = generateGraph(functionName, flow)
                            if filename:
                                filepath.append(filename)
    ...
    if startStopCnt != 0:
        errorMessage = ('Graph was not generated! Please check if you ' +
                        'put "fc:startStop" at the end of the function.')
        print(errorMessage)
...
def parseCLIArguments():
    ...
    parser.add_argument('--func',
                        ...
                        help=('With this argument you can create graph for ' +
                              'only specified functions in source file.'))
    parser.add_argument('-l', '--lang',
                        dest='lang',
                        required=True,
                        help=('With this argument you define programming language ' +
                              'you are using. "C" is for C-like comments and "python" ' +
                              'is for python like comments.'))
